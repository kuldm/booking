...
async def get_all_rooms(
    ...
    return await db.rooms.get_all_rooms(
        title=title, limit=per_page, offset=per_page * (pagination.page - 1)
    )
...
@router.get(
    ...
    description="<h3>В этой ручке мы получаем список всех свободных номеров конкретного отеля на конкретные даты. Можем фильтроваться по title<h3>",
)
...
async def create_room(
    ...
    room_data: RoomAddRequest = Body(
        openapi_examples={
            "1": {
                "summary": "Простая комната рядом с отелем",
                "value": {
            ...
            "5": {
                "summary": "Дубай королевская комната",
                "value": {
